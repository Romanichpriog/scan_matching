...
void CircleOdometry::loadParamsFromYAML(const std::string &file_name) {
    ...
    std::ifstream in("/app/PCM/circle_odom_ws/src/circle_odometry/config/data.txt");
    std::cin.rdbuf(in.rdbuf());

    for (int i = 0; i < 1622; ++i) {
        cout << "New source " << i << endl;
        newTarget();
        run();
    }
...
void CircleOdometry::FileToCloud2Msg(sensor_msgs::PointCloud2& pcl2_laser)
{
...
void CircleOdometry::newTarget() {
    frame_cnt = 0;
}


void CircleOdometry::hello() {
    cout << "Hello" << endl;
}
...
void CircleOdometry::process() {
    frame_cnt ++;
    std::ofstream outFile("/app/PCM/circle_odom_ws/src/circle_odometry/config/result.txt", std::ios::app);
